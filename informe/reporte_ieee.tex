...
resuelve además con CP-SAT, que certifica cinco de los seis casos: el mínimo es $1$ caja
activa para $R=1$, $4$ para $R=2$ y $9$ para $R=3$ con $z=4$ (el caso $R=3$, $z=8$ queda
demostrado en el intervalo $[8,10]$). Traducido a complejidad de datos, el ataque
...
$120$~s por instancia, sobre las seis combinaciones $z\in\{4,8\}\times R\in\{1,2,3\}$. El
...
\midrule
4 & 1 & $1$ cert.      & $1$ cert. (0,8 s)  & $1$ \\
4 & 2 & $9$, gap 100\% & $4$ cert. (2,9 s)  & $4$ \\
4 & 3 & $18$, gap 100\% & $9$ cert. (68 s)  & $9$ \\
8 & 1 & $1$ cert.      & $1$ cert. (2,1 s)  & $1$ \\
8 & 2 & $4$, gap 100\% & $4$ cert. (9,2 s)  & $4$ \\
8 & 3 & sin solución   & $[8,10]$ (260 s)   & ? \\
\bottomrule
\end{tabular}
\end{table}
...
El resultado es concluyente (Tabla~\ref{tab:comp}): CP-SAT certifica cinco de los seis
casos, con tiempos entre $0{,}8$ y $68$~s, incluidos aquellos en que el MILP no cerraba en
$120$~s. Sólo $R=3$, $z=8$ queda abierto, con el mínimo demostrado en el intervalo
$[8,10]$: existe una trayectoria de $10$ cajas y se ha probado que ninguna tiene menos de
$8$.
...
\emph{defensor} (ninguna trayectoria tiene menos de $n$ cajas). Los cinco casos
certificados proporcionan ambas simultáneamente.
...
$20$--$29$ & 3 & $9$ (exacto, $z{=}4$) & $2^{-18}$       & $2^{18}$ \\
           &   & $\ge 8$ ($z{=}8$)     & $\le 2^{-16}$   & $\ge 2^{16}$ \\
\bottomrule
\end{tabular}
\end{table}

El crecimiento es superlineal. Al pasar de una a tres rondas, la complejidad de datos del
ataque diferencial óptimo asciende de $2^{2}$ a $2^{18}$ pares, es decir, de unas pocas
unidades a $\sim 2{,}6\times10^{5}$: dieciséis órdenes binarios de magnitud. La razón
...
pares). Segundo, el caso $R=3$, $z=8$ permanece sin cerrar en $[8,10]$. Tercero, el
reescalado de los desplazamientos de $\rho$ (Sec.~\ref{sec:lineal}) degrada la difusión
...
\begin{enumerate}
\item El mínimo de cajas-S activas está certificado en cinco de los seis casos: $1$ para
$R=1$, $4$ para $R=2$ (ambos tamaños de palabra) y $9$ para $R=3$ con $z=4$. El caso $R=3$,
$z=8$ queda demostrado en $[8,10]$. Una ronda es criptográficamente insuficiente.
...
Como trabajo futuro queda cerrar el caso $R=3$, $z=8$, para lo que cabe explotar la
estructura del \emph{column-parity kernel} de Keccak o instrumentar solvers SAT
especializados. Un camino complementario, si se deseara mantener el enfoque MILP, es
sustituir el encoding \emph{big-M} de $\chi$ aprovechando que el conjunto de diferencias de
salida válidas es, para cada diferencia de entrada, un subespacio afín de $\Fdos^5$ de
dimensión $2$, $3$ o $4$; ello permitiría reemplazar las $317$ variables de selección por
unas pocas ecuaciones lineales. Sería igualmente pertinente extender el análisis a
criptoanálisis lineal y evaluar el canal lateral temporal introducido por el número
variable de rondas.
...
\begin{center}
\url{https://github.com/USUARIO/keccak-milp-dinamico}
\end{center}
\noindent\emph{(Sustituir por la URL definitiva del repositorio antes de la entrega.)}
