...
# ---------------------------------------------------------------------------
# Envolvente convexa de la DDT: la formulación escalar es inválida
# ---------------------------------------------------------------------------
def test_hull_escalar_admite_transiciones_invalidas():
    """REGRESIÓN. Colapsar la envolvente convexa en dos ecuaciones escalares
    con potencias de dos deja la restricción de la DDT vacía.

    Se toma la transición (a=3, b=1), que NO pertenece a la DDT, y se comprueba
    que la formulación escalar la admite: existe una combinación convexa de los
    pares válidos cuyos valores enteros son exactamente 3 y 1. Ese es el defecto
    que hacía que el modelo devolviera valores por debajo del óptimo real.
    """
    import pulp
    trans = ddt_transiciones()
    pares = [(a, b) for a, bl in trans.items() for b in bl]
    assert 1 not in trans[3], "el supuesto de la prueba ya no se cumple"

    prob = pulp.LpProblem("escalar", pulp.LpMinimize)
    lam = pulp.LpVariable.dicts("lam", range(len(pares)), lowBound=0, cat="Continuous")
    prob += pulp.lpSum(lam.values()) == 1
    prob += pulp.lpSum(lam[j] * pares[j][0] for j in range(len(pares))) == 3
    prob += pulp.lpSum(lam[j] * pares[j][1] for j in range(len(pares))) == 1
    prob += 0
    prob.solve(pulp.PULP_CBC_CMD(msg=0))
    assert pulp.LpStatus[prob.status] == "Optimal", \
        "la formulación escalar deberia admitir la transicion invalida (3 -> 1)"
    print("  [ok] la formulacion escalar admite (3 -> 1), que no esta en la DDT")


def test_hull_por_componentes_rechaza_transiciones_invalidas():
    """La envolvente impuesta bit a bit sí es exacta: rechaza (a=3, b=1).

    Para un conjunto S de puntos 0/1 se cumple conv(S) inter {0,1}^n = S, de modo
    que la formulación por componentes describe exactamente las transiciones
    válidas.
    """
    import pulp
    trans = ddt_transiciones()
    ...
    prob = pulp.LpProblem("componentes", pulp.LpMinimize)
    lam = pulp.LpVariable.dicts("lam", range(len(pares)), lowBound=0, cat="Continuous")
    prob += pulp.lpSum(lam.values()) == 1
    for i in range(5):
        prob += pulp.lpSum(lam[j] * ((pares[j][0] >> i) & 1)
                           for j in range(len(pares))) == ((3 >> i) & 1)
        prob += pulp.lpSum(lam[j] * ((pares[j][1] >> i) & 1)
                           for j in range(len(pares))) == ((1 >> i) & 1)
    prob += 0
    prob.solve(pulp.PULP_CBC_CMD(msg=0))
    assert pulp.LpStatus[prob.status] == "Infeasible", \
        "la formulacion por componentes no deberia admitir (3 -> 1)"
    print("  [ok] la formulacion por componentes rechaza (3 -> 1)")


def test_consolidacion_monotona_de_cotas():
    """m(R) es no decreciente en R, luego las cotas inferiores se propagan.

    Truncar una trayectoria de R+1 rondas produce una de R rondas con un número
    de cajas activas menor o igual, de donde m(R+1) >= m(R).
    """
    try:
        from cpsat_keccak import consolidar
    except ImportError as e:
        if "undefined symbol" in str(e):
            print("  [omitido] conflicto highspy/ortools en este proceso")
        else:
            print("  [omitido] OR-Tools no instalado")
        return
    entrada = [
        {"z": 4, "R": 1, "cota_inferior": 1, "cota_superior": 1},
        {"z": 4, "R": 2, "cota_inferior": 4, "cota_superior": 4},
        {"z": 4, "R": 3, "cota_inferior": 9, "cota_superior": 9},
        {"z": 4, "R": 4, "cota_inferior": 6, "cota_superior": 14},
    ]
    salida = {d["R"]: d for d in consolidar(entrada)}
    assert salida[4]["cota_inferior_consolidada"] == 9, "la cota de R=3 debe propagarse"
    assert salida[4]["origen_cota"] == 3
    assert salida[3]["exacto"] is True
    assert salida[4]["exacto"] is False
    print("  [ok] la cota inferior de R=3 (9) se propaga a R=4")
